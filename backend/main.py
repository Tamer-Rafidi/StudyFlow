...
async def test_openai_key(request: TestAPIKeyRequest):
    ...
    try:
        ...
        try:
            ...
            # This just lists available models, which is a cheap/fast operation
...
async def upload_pdf(
    ...
    def push_progress(stage: str, progress_pct: int, message: str):
        ...
        # Use call_soon_threadsafe to be safe if called from thread (we call from async and from executor)
    ...
    # Orchestrator: async function that uses run_in_executor for blocking parts and sends heartbeats
    async def process_async():
        loop = asyncio.get_running_loop()

        try:
            # Stage: uploading (already done)
            push_progress("uploading", 10, "Uploading file...")
            push_progress("uploading", 20, "File uploaded successfully")

            # Stage: extracting - run extractor in executor
            ...
            while not extract_future.done():
                ...
                # flexible heartbeat progress between 25 -> 40
            ...
            result = await extract_future  # blocking result now available
            ...
            # Create DB record quickly in executor (so it doesn't block)
            ...
            # Prepare text for AI (already truncated logic same as before)
            text = result.get('full_text', '')

            # Stage: summarizing - run in executor and heartbeat while waiting
            ...
            # Heartbeat during LLM summary: tick progress 55 -> 70
            start_ts = loop.time()
            ...
            # Save summary file and DB record (run in executor)
            ...
            # Stage: generating flashcards - run generator in executor with heartbeats
            ...
            # Heartbeat for flashcard generation (progress 75 -> 90)
            start_ts = loop.time()
            ...
            # Save flashcards to DB (do not block event loop)
            ...
            # For a smoother UX, emit incremental saving progress approximations
            for i in range(1, saved_count + 1):
                # approximate saved ratio; we can't see individual saves (they happened in executor),
                # but still emit a small ramp to 99% so UI looks responsive
    ...
    # Start the orchestrator as a background asyncio task (it uses run_in_executor internally)
    ...
    async def generate():
        try:
            while True:
                ...
                # slight pause to allow chunk flush; necessary in some server setups
...
def delete_course_endpoint(course_code: str):
    ...
    try:
        ...
        # Delete all documents (cascade will handle flashcards and summaries)
...
def get_document_summary(document_id: int):
    ...
    try:
        ...
        # Extract topics (very basic - just look for capitalized words)
        import re
    ...
    try:
        ...
        for idx, question in enumerate(questions):
            ...
            if question['type'] == 'short_answer':
                correct_answer = question.get('sample_answer')  # ← Use sample_answer
            else:
            ...
            if question['type'] == 'true_false':
                ...
                if user_answer and correct_answer:
                    ...
                    if len(user_lower) < 10:
                        ...
                        # Filter out very short key points (less than 3 chars)
                        ...
                        # 1. User answer significantly overlaps with sample (at least 20% of sample words)
                        # 2. OR user answer contains required key points
                        sample_words = set(correct_lower.split())
                        user_words = set(user_lower.split())
                        overlap = len(sample_words.intersection(user_words))
                        
                        is_correct = (
                            overlap >= len(sample_words) * 0.2 or  # 20% word overlap
                            has_key_points
                        )
            ...
            results.append({
                ...
                "correct_answer": correct_answer,  # Now this will show the sample_answer
...
def get_course_statistics(course_code: str):
    ...
    try:
        ...
        return {
            ...
            "recent_activity": []  # You can implement this later
        }
...
async def clear_all_data(db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Delete all data from database tables in correct order (respecting foreign keys)
        
        # 1. Delete flashcards first (they reference documents)
        flashcard_count = db.query(Flashcard).count()
        db.query(Flashcard).delete()
        print(f"Deleted {flashcard_count} flashcards")
        
        # 2. Delete summaries (they reference documents)
        ...
        # 3. Delete documents (they reference courses)
